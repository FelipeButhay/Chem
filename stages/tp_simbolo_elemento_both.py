...
variations = [None, None, [2, 1, f"Escribir el simbolo del elemento señalado, tocar ENTER para confirmar. Si no lo sabe puede tocar la flecha para saltarlo."], 
                          [1, 2, f"Escribir el nombre del elemento señalado, tocar ENTER para confirmar. Si no lo sabe puede tocar la flecha para saltarlo."]]
...
def game(mouse_data: list, game_data: list, grid: list[list], grid_data: list) -> list:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.dict['unicode'] == '\x1b':
                ...
                if tp.normalize_words(element[variations[game_mode][0]]) == tp.normalize_words(cword):
                    new_state = 1
                else:
                    new_state = 2
    ...
    if element != None: 
        element_txt = tp.font[0].render(element[variations[game_mode][1]], True, tuple(50 for x in range(3)))
        tp.screen.blit(element_txt,(tp.SX*.25, tp.SY*0.3))
    ...
    hint_txt = tp.font[3].render(variations[game_mode][2], True, tuple(50 for x in range(3)))
