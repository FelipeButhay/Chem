...
def game(mouse_data: list, game_data: list, grid: list[list], grid_data: list) -> list:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.dict['unicode'] == "\x08" and len(cword) > 0:
                cword = cword[0:-1]
            elif event.dict['unicode'].isalpha() and len(cword) < 16:
    ...
    cword_txt = tp.font[0].render(cword, True, tuple(50 for x in range(3)))
    ...
    hint_txt = tp.font[3].render(
        f"Escribir el {nom_or_sim(nom_sim)} del elemento señalado, tocar ENTER para confirmar.", True, tuple(50 for x in range(3)))
